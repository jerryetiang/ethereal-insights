...
const Comment: React.FC<CommentProps> = ({
  ...
  <article className="p-6 text-base bg-zinc-100 rounded-lg dark:bg-zinc-900 mb-4">
    <footer className="flex justify-between items-center mb-2">
      <div className="flex items-center">
        <p className="inline-flex items-center mr-3 text-sm text-zinc-900 dark:text-white font-semibold">
          <Image
            ...
            src={`/blog-samples/freelancer.png`}
            alt={author.name}
            width={100}
            height={100}
          />
      ...
      {/* Dropdown menu */}
      <div
        id={`dropdownComment${author.id}`}
        className="hidden z-10 w-36 bg-white rounded divide-y divide-zinc-100 shadow dark:bg-zinc-700 dark:divide-zinc-600"
      >
        <ul
          className="py-1 text-sm text-zinc-700 dark:text-zinc-200"
          aria-labelledby={`dropdownMenuIconHorizontalButton${author.id}`}
        >
          <li>
            <a
              href="#"
    ...
    <div className="flex items-center mt-4 space-x-4">
      <button
        type="button"
        className="flex items-center text-sm text-zinc-500 hover:underline dark:text-zinc-400 font-medium"
      >
        <svg
